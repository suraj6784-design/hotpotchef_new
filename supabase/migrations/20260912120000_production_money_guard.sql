-- Production money guard: server-quoted delivery fee, service-role-only order insert.
...
CREATE OR REPLACE FUNCTION public.cart_item_is_delivery(p_item jsonb)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN lower(coalesce(
      p_item->>'selected_service_type',
      p_item->>'selectedServiceType',
      p_item->>'service_type',
      p_item->>'serviceType',
      ''
    )) ~ 'pickup|pick up|dine' THEN false
    ELSE true
  END
$$;
...
BEGIN
  IF p_lat1 IS NULL OR p_lng1 IS NULL OR p_lat2 IS NULL OR p_lng2 IS NULL THEN
    RETURN 0;
  END IF;
  IF p_lat1 = 0 OR p_lng1 = 0 OR p_lat2 = 0 OR p_lng2 = 0 THEN
    RETURN 0;
  END IF;
  v_a := sin(radians((p_lat2 - p_lat1) / 2)) ^ 2
    + cos(radians(p_lat1)) * cos(radians(p_lat2))
      * sin(radians((p_lng2 - p_lng1) / 2)) ^ 2;
  v_a := LEAST(1, GREATEST(0, v_a));
  RETURN 6371 * 2 * atan2(sqrt(v_a), sqrt(1 - v_a));
END;
$$;

CREATE OR REPLACE FUNCTION public.delivery_fee_for_distance_km(p_km numeric)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_km IS NULL OR p_km < 0 THEN 30
    WHEN p_km <= 3 THEN 30
    ELSE 30 + ceil(p_km - 3) * 10
  END
$$;

CREATE OR REPLACE FUNCTION public.quote_checkout_delivery_fee(
  p_items jsonb,
...
BEGIN
  ...
  LOOP
    IF public.cart_item_is_delivery(v_item) THEN
      v_has_delivery := true;
    END IF;
    BEGIN
      v_chef_id := NULLIF(COALESCE(v_item->>'chef_id', v_item->>'chefId'), '')::uuid;
    EXCEPTION WHEN OTHERS THEN
      v_chef_id := NULL;
    END;
    IF v_chef_id IS NOT NULL AND NOT v_chef_id = ANY (v_chef_ids) THEN
      v_chef_ids := array_append(v_chef_ids, v_chef_id);
    END IF;
  END LOOP;
  ...
  IF p_drop_lat IS NULL OR p_drop_lng IS NULL OR p_drop_lat = 0 OR p_drop_lng = 0 THEN
    RETURN 30;
  END IF;

  IF array_length(v_chef_ids, 1) IS NULL THEN
    RETURN 30;
  END IF;

  FOREACH v_chef_id IN ARRAY v_chef_ids
  LOOP
    SELECT NULLIF(to_jsonb(u)->>'lat', '')::numeric, NULLIF(to_jsonb(u)->>'lng', '')::numeric
      INTO v_lat, v_lng
    FROM users u
    WHERE u.id = v_chef_id;
    IF v_lat IS NULL OR v_lng IS NULL THEN
      v_total := v_total + 30;
    ELSE
      v_km := public.haversine_km(v_lat, v_lng, p_drop_lat, p_drop_lng);
      IF v_km <= 0 THEN
        v_total := v_total + 30;
      ELSE
        v_total := v_total + public.delivery_fee_for_distance_km(v_km);
      END IF;
    END IF;
  END LOOP;
...
REVOKE ALL ON FUNCTION public.quote_checkout_delivery_fee(jsonb, numeric, numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.quote_checkout_delivery_fee(jsonb, numeric, numeric) TO authenticated, service_role;
