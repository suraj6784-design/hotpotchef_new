-- Close leftover money / loyalty / group-cart cycles:
-- cart total RPC, driver tip on payout, coins gate, catering quote, loyalty on deliver.

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS driver_payout numeric DEFAULT 0;
...
CREATE OR REPLACE FUNCTION public.calculate_cart_total(p_items jsonb)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
AS $$
...
REVOKE ALL ON FUNCTION public.calculate_cart_total(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.calculate_cart_total(jsonb) TO authenticated, service_role;
