...
import { isDeliveredLike, isOutForDelivery, isReadyForPickup, parseOrderStatus } from "./order_status.ts"
...
Deno.test("delivered-like accepts Title Case and snake_case", () => {
  assertEquals(isDeliveredLike("Delivered"), true)
  assertEquals(isDeliveredLike("delivered"), true)
  assertEquals(isDeliveredLike("Completed"), true)
  assertEquals(isDeliveredLike("Available"), false)
})
